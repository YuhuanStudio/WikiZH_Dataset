...
| 檢查 | 結果 |
|---|---|
| 程式碼區塊與原始碼逐字相符 | 繁體 94.122%；簡體 93.961% |
| 公式與原始碼逐字相符 | 繁體 98.661%；簡體 98.669% |
| 繁簡兩版結構對等 | ID 序列完全一致；抽驗 20,000 篇為 0 個阻斷差異 |
| 空正文／模板／HTML／空白／私有區字元等 16 類硬性缺陷 | 0 |
...
# 简体版卡片的文案直接写，不用转换器产生。
#
# 这个专案本身就在做繁简转换，很清楚自动转换有极限：术语（资讯框／信息框、
# 栏位／字段、汇入／导入）不是换字形就对，而说明文件的读者是人。文案不长，
# 写两份比转出一份半对不对的可靠。
_CARD_CHANGES_CN = """
## `2607` 以前与 `2608` 起的差异

**整条处理流程重写过**，这不是单纯的月度刷新——数据的形状变了。
若你正在用 `2607` 或更早的版本，升级前请先看这张表：

| | `2607` 以前 | `2608` 起 |
|---|---|---|
| 条数／单位 | 371 万行（**段落级**）| **148 万篇**（文档级，一条目一条）|
| 文件格式 | JSON 分段（`wiki_pretrain_part*.json`）| **Parquet 分片** |
| 章节结构 | 切碎后不存在 | 保留 `##` 层级 |
| 表格 | 整块丢弃 | **转成文字**（表格占可见内容约 15%）|
| 代码 | 散文规则套上去，缩进与换行被破坏 | **逐字保留**，`` ``` `` 围栏 |
| 公式 | 丢弃 | **保留 LaTeX**，以 `$` 包住 |
| 信息框 | 整块丢弃 | 事实抽成 `## 基本资料`，字段名取自维基的模板定义 |
...
**数据集的核心指标是“原文的东西丢了多少”**——残留标记可以靠把整段删掉变成 0，
那不是干净。扫一次 dump，从源码取出可验证的内容项目，回头去两版正文里找。
只比两版**都收录**的 1,367,033 篇：

| 项目 | 样本数 | `2607` 留住 | `2608` 留住 |
|---|---|---|---|
| 公式 | 91,552 | 0.0% | **99.2%** |
| 代码块 | 6,261 | 2.9% | **99.0%** |
| 表格单元格 | 2,563,309 | 2.5% | **51.4%** |
| 列表项 | 5,522,474 | 30.9% | **70.2%** |
| 信息框字段值 | 6,721,977 | 5.1% | **30.7%** |
| 段落句子 | 5,564,852 | 59.0% | **74.8%** |
| **合计** | **20,470,425** | **26.4%** | **56.3%** |

连纯散文，`2607` 也只留住 59%——四成的正文句子不见了。

`2608` 的 56.3% 不是满分，但剩下的有明确归属：信息框字段值的探针含大量版面设置
（图片文件名、尺寸、色码），表格探针含样式属性（以人工挑过的探针量是 92.3% OK），
段落则是参考资料／外部链接等章节按设计整节不收。公式与代码的探针最干净，
99% 可视为保留率的上界参考。

条目收录：1,419,539 篇 → **1,482,182 篇**（+62,643）。

### 实测对照（全量，非抽样）

把公开的 `2607` 数据整份抓下来（2.9 GB），与 `2608` 的全部 148 万篇套完全相同的
判准逐条扫描：

| | `2607` 以前 | `2608` 起 | |
|---|---|---|---|
| 条数 | 3,711,316 行 | 1,482,182 篇 | 单位从段落改成整篇 |
| **总字符** | **9.45 亿** | **17.56 亿** | **×1.86** |
| 每条平均字符 | 255 | 1,185 | |

**同一份维基百科，新版多出 8.1 亿字符**——多的不是重复，是原本被丢掉的表格、
信息框事实、列表、代码与公式。
...
`2607` 以前的版本仍存放在本 repo 的 `YYMM/` 月份文件夹里，需要的话可以直接读旧路径。
"""

_CARD_COMMON_CN = """
## 这份数据怎么来的

从维基百科官方 dump 解析，每月更新。原则是**只移除确定是标记的东西，
绝不删除自然语言内容**：模板展开而非删除、表格转文字而非丢弃、代码与公式
逐字保留、图库图说当内容。完整说明与质量数据见
[GitHub 项目](https://github.com/YuhuanStudio/WikiZH_Dataset)。

## 质量

每一轮都跑全量审核，数字公开在项目 README：

| 检查 | 结果 |
|---|---|
| 代码块与源码逐字相符 | 繁体 94.122%；简体 93.961% |
| 公式与源码逐字相符 | 繁体 98.661%；简体 98.669% |
| 繁简两版结构对等 | ID 序列完全一致；抽验 20,000 篇为 0 个阻断差异 |
| 空正文／模板／HTML／空白／私有区字符等 16 类硬性缺陷 | 0 |
...
def _pretrain_card(lang, version, dump_date):
    if lang == 'cn':
        return _card_header('zh-Hans', ['text-generation'],
                            PRETRAIN_DATA_GLOB) + f"""
# 中文维基百科纯文本数据集（简体中文）

一个条目一条记录，保留章节结构，可直接用于语言模型预训练。

> 📅 **当前版本**：`{version}`（维基百科 dump 日期：{format_dump_date(dump_date)}）
...
| `id` | 维基百科条目 ID |
| `title` | 条目标题 |
| `url` | 条目原文网址（用未经繁简转换的原始标题组成，直接命中不跳转）|
| `text` | 条目全文。第一行为标题，章节以 `##` 标示，列表为 `- `，表格行以 `｜` 分隔，代码用 `` ``` `` 围栏，公式保留 LaTeX 并以 `$` 包住 |

**不含图说**——图说说的是“这张图是什么”，不是条目本身的叙述，它在
[图文配对版](https://huggingface.co/datasets/{IMAGE_REPO}) 与
[图文交错版](https://huggingface.co/datasets/{OMNI_REPOS['cn']}) 里。
...
被丢掉的救不回来。
""" + _CARD_CHANGES_CN + _CARD_COMMON_CN

    name = '台灣正體中文'
    return _card_header('zh-Hant', ['text-generation'], PRETRAIN_DATA_GLOB) + f"""
# 中文維基百科純文字資料集（{name}）
...
def _omni_readme(lang, version, dump_date):
    if lang == 'cn':
        return _card_header('zh-Hans', ['image-to-text', 'text-generation'],
                            PRETRAIN_DATA_GLOB) + f"""
# 中文维基百科图文交错数据集（简体中文）

正文与图片**交错**排列：`text` 里的每个 `<image>` 占位符对应 `images` 数组的
...
| 字段 | 说明 |
|---|---|
| `id` / `title` / `url` | 同纯文本版，可用 `id` 对应 |
| `text` | 条目全文，图片位置以 `<image>` 标示 |
| `images` | 图片数组：`url` / `file_name` / `caption` / `alt` |

**不变量**：`text.count("<image>") == len(images)`，全量实测 0 误差。章节被丢弃时
它的图片一并丢弃，两者永远对得上。

与[纯文本版](https://huggingface.co/datasets/{PRETRAIN_REPOS['cn']})的差别只有
图片处理：纯文本版把标记整行移除、并清掉“拿掉图片后就空掉”的章节；这一版
保留它们——只有地图的一节对纯文本模型是空标题，对多模态模型是有价值的图文对。

> ⚠️ 图片授权各不相同（CC BY-SA、公有领域，也有合理使用的非自由图片）。
> 授权信息不在 dump 里，本数据集**不含授权字段**，使用前请自行向 Commons 查证。
""" + _CARD_CHANGES_CN + _CARD_COMMON_CN

    name = '台灣正體中文'
    return _card_header('zh-Hant', ['image-to-text', 'text-generation'],
                        PRETRAIN_DATA_GLOB) + f"""
# 中文維基百科圖文交錯資料集（{name}）
...
def run_upload(base_dir, dump_date, langs=('tw', 'cn'), token=None, dry_run=False,
    ...
    if skip_checks:
        ...
        # 卡片的字體也要與 repo 相符。簡體 repo 掛一張繁體說明，程式不會報錯、
        # dry-run 也照樣全綠——這種事只有讀的人會發現，所以讓機器來查：
        # 判準用 zhconv 的「另一側專用字」，與資料集的字體純度稽核同一套。
        import zhconv
        tables = zhconv._load()
        wrong = {'tw': zhconv._target_only_chars(tables['zh2Hant']),
        ...
        for repo_id in card_repos:
            ...
            if bad:
                script_check.error(
                    f'{repo_id}（應為{"繁" if lang == "tw" else "簡"}體）'
                    f'含 {len(bad)} 種另一側的字：{"".join(bad[:12])}')
        if not script_check.errors:
            script_check.info(f'{len(card_repos)} 張卡片的字體皆與 repo 相符')
        results.append(script_check)
