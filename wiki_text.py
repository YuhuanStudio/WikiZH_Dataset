...
# 已失去 `[[...]]` 外殼、但仍獨佔一行的 namespace 目標也是確定的標記。
# 字面教學範例會位於 nowiki／程式碼區，已由逐字遮罩保護。
_ORPHAN_NAMESPACE_LINE_RE = re.compile(
    r'(?im)^\s*(?:Template|Category|模板|分類|分类)\s*:[^\n]*$')
...
def strip_leftover_markup(text):
    ...
    text = _ORPHAN_NAMESPACE_LINE_RE.sub('', text)
...
SKIP_SECTION_KEYWORDS = [
         # 參考相關
            ...
            # 外部連結
            "外部連結", "外部链接", "外部連接", "外部连接", "外部連結和參考資料", "外部链接和参考资料", "相關連結", "相关链接", "連結", "链接", "網站連結", "网站链接", "外部資源", "外部资源",
